...
func TestLoadSynonymDict(t *testing.T) {
	...
	if dict.Size() != 5 {
		t.Errorf("Size() = %d, want 5", dict.Size())
	}
}
...
func TestSynonymLookup_English(t *testing.T) {
	path := createTestSynonymFile(t)
	dict, err := LoadSynonymDict(path)
	if err != nil {
		t.Fatalf("LoadSynonymDict() error = %v", err)
...
func TestParseJiebaClause_WithSynonym_English(t *testing.T) {
	path := createTestSynonymFile(t)
	p := New(WithSynonym(path))
	defer p.Close()
...
func TestParseJiebaClause_SynonymFileNotExist(t *testing.T) {
	...
	p := New(WithSynonym("/不存在/path/synonym.txt"))
	defer p.Close()
