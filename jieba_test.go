...
func TestParser_ParseJiebaClause(t *testing.T) {
	p := New()
	defer p.Close()

	got := p.ParseJiebaClause("我爱中国")
	want := `"我" AND "爱" AND "中国"`
	if got != want {
		t.Errorf("Parser.ParseJiebaClause(%q)\n  got:  %s\n  want: %s", "我爱中国", got, want)
	}
}

func TestParseJiebaClause_TraditionalChinese(t *testing.T) {
	got := ParseJiebaClause("中華人民共和國")
	want := `"中华人民共和国"`
	if got != want {
		t.Errorf("ParseJiebaClause(繁体)\n  got:  %s\n  want: %s", got, want)
	}
}

// TestParseJiebaClause_Concurrent 验证包级 API 在并发调用下不会 panic。
func TestParseJiebaClause_Concurrent(t *testing.T) {
	var wg sync.WaitGroup
	queries := []string{"我爱中国", "hello", "周杰伦 Jay Chou", "test123", "2024年"}
	for i := 0; i < 20; i++ {
		wg.Add(1)
		go func(q string) {
			defer wg.Done()
			got := ParseJiebaClause(q)
			if got == "" {
				t.Errorf("concurrent ParseJiebaClause(%q) returned empty", q)
			}
...
// TestParser_CloseIdempotent 验证 Close 多次调用不会 panic。
func TestParser_CloseIdempotent(t *testing.T) {
	p := New()
	p.Close()
	p.Close() // 不应 panic
}
